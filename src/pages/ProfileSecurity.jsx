      ...
      <div className="px-6 space-y-3 mt-2">
        ...
        <GlassCard className="overflow-hidden">
          <button onClick={() => setSection(section === 'password' ? null : 'password')}
            ...
            <div className="flex-1 text-left">
              <p className="text-sm font-medium text-mora-white">Change Password</p>
              <p className="text-xs text-mora-neutral/50 mt-0.5">Update your account password</p>
            </div>
            <ChevronRight className={`w-4 h-4 text-mora-neutral/30 transition-transform ${section === 'password' ? 'rotate-90' : ''}`} />
          </button>
          {section === 'password' && (
        ...
        <GlassCard className="p-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-mora-white">Two-Factor Auth</p>
        ...
        <GlassCard className="overflow-hidden">
          <button onClick={() => setSection(section === 'activity' ? null : 'activity')}
            ...
            <div className="flex-1 text-left">
              <p className="text-sm font-medium text-mora-white">Login Activity</p>
              <p className="text-xs text-mora-neutral/50 mt-0.5">See your recent sessions</p>
            </div>
            <ChevronRight className={`w-4 h-4 text-mora-neutral/30 transition-transform ${section === 'activity' ? 'rotate-90' : ''}`} />
          </button>
          {section === 'activity' && (
            <div className="border-t border-white/5">
              {loginActivity.map((s, i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-3 border-b border-white/5 last:border-0">
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${s.current ? 'bg-emerald-400' : 'bg-mora-neutral/30'}`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-mora-white">{s.device}</p>
                    <p className="text-xs text-mora-neutral/50">{s.location} · {s.time}</p>
                  </div>
