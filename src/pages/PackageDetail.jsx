...
const Section = ({ title, icon: Icon, children }) => (
  <GlassCard className="p-5">
    <h2 className="text-sm font-semibold text-mora-primary mb-3 flex items-center gap-2">
    ...
    <div className="animate-fade-in" style={{ paddingBottom: 24 }}>
      ...
      <div className="relative h-60 bg-mora-primary">
        ...
        <div className="absolute top-3 right-4 flex gap-2">
          ...
          {off != null && <span className="chip-glass !bg-mora-gold/90 !border-white/30">{off}% off</span>}
        </div>
      ...
      <div className="px-6 -mt-4 relative z-10 space-y-4">
        ...
        <GlassCard className="p-5">
          <div className="flex items-end justify-between gap-3">
            <div>
              {off != null && <p className="text-xs text-mora-neutral/50 line-through leading-none">{formatIDR(pkg.price_before)}</p>}
              <p className="stat-value text-2xl font-display font-bold text-gold">{formatIDR(pkg.price)}</p>
              <p className="text-[11px] text-mora-neutral/70">per person</p>
            </div>
            {pkg.slots_left > 0 && (
          ...
          {/* Travellers */}
          <div className="flex items-center justify-between gap-3 mt-4 pt-4 border-t border-mora-primary/10">
            <div className="flex items-center gap-2 text-sm text-mora-primary">
          ...
          <p className="text-[11px] text-mora-neutral/60 mt-2">
        ...
        {(pkg.summary || pkg.description) && (
          <Section title="About this package" icon={Sparkles}>
            <p className="text-sm text-mora-neutral leading-relaxed">{pkg.description || pkg.summary}</p>
          </Section>
        )}
          ...
          <Section title="What's included" icon={Check}>
            <ul className="space-y-2">
              {(pkg.includes || []).map((x, i) => (
                <li key={`i${i}`} className="flex items-start gap-2.5 text-sm text-mora-neutral">
                  <Check className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />{x}
                </li>
              ))}
              {(pkg.excludes || []).map((x, i) => (
                <li key={`e${i}`} className="flex items-start gap-2.5 text-sm text-mora-neutral/60">
                  <X className="w-4 h-4 text-mora-neutral/40 shrink-0 mt-0.5" />{x}
                </li>
              ))}
            </ul>
          </Section>
            ...
            <ol className="space-y-3.5">
              {pkg.itinerary.map((d, i) => (
                <li key={i} className="flex gap-3">
                  <span className="w-8 h-8 rounded-xl bg-mora-gold/10 text-gold flex items-center justify-center text-xs font-display font-bold shrink-0">
                    {d.day ?? i + 1}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-mora-primary">{d.title}</p>
                    {d.detail && <p className="text-xs text-mora-neutral/70 leading-relaxed mt-0.5">{d.detail}</p>}
                  </div>
                </li>
              ))}
            </ol>
      ...
      <div className="sticky bottom-[92px] z-20 px-6 mt-5">
        <div className="glass-nav shadow-float rounded-2xl p-3 flex items-center gap-3">
          <div className="min-w-0">
            <p className="text-[10px] text-mora-neutral/60 uppercase tracking-wider leading-none">Total · {pax} traveller{pax > 1 ? "s" : ""}</p>
