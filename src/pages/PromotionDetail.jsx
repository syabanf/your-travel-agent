...
export default function PromotionDetail() {
  ...
  if (!item) return (
    <div className="animate-fade-in"><PageHeader title="Not found" showBack /><p className="px-6 text-sm text-mora-neutral/70">This item is no longer available.</p></div>
  );
  ...
  return (
    <div className="animate-fade-in pb-28">
      {/* Hero */}
      <div className="relative h-56 bg-mora-primary">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5">
          <div className="flex items-center gap-2 mb-1.5">
            ...
            {item.discount ? <span className="text-[10px] font-bold bg-mora-gold text-white px-2.5 py-1 rounded-full">{item.discount}% OFF</span> : null}
          </div>
      ...
      <div className="px-6 -mt-4 relative z-10 space-y-4">
        <GlassCard className="p-5">
          <p className="text-sm text-mora-neutral leading-relaxed">{item.description}</p>
          <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-mora-primary/10">
        ...
        <GlassCard className="p-5">
          <h2 className="text-sm font-semibold text-mora-primary mb-3">Good to know</h2>
          <ul className="space-y-2">
            {perks.map((p) => (
              <li key={p} className="flex items-center gap-2.5 text-sm text-mora-neutral">
