          ...
          return (
            <Link key={booking.id} to={`/booking/${booking.id}`} className="block press-spring">
              <div className="card-modern rounded-2xl p-3.5 flex items-center gap-3.5 hover:shadow-lift transition-shadow">
                <div className="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0 bg-gradient-to-br from-ich-gold/15 to-ich-gold/[0.04] ring-1 ring-ich-gold/10">
                  <Icon className="w-[18px] h-[18px] text-gold" strokeWidth={1.8} />
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-semibold text-ich-primary truncate">{booking.title}</h4>
                  <p className="text-xs text-ich-neutral/70 mt-0.5 truncate">
                ...
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full capitalize flex-shrink-0 ${
                  booking.status === "confirmed"
                    ...
                    : "bg-ich-gold/10 text-gold border border-ich-gold/20"
                }`}>
                  {booking.status}
                </span>
              </div>
