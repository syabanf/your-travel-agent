    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        <div>
          ...
          <h1 className="text-2xl font-display font-bold text-mora-primary">Holiday Packages</h1>
          <p className="text-sm text-mora-neutral mt-0.5">Ready-made trips travellers can buy in the app. Drafts stay hidden until you publish them.</p>
        </div>
      ...
      {items == null ? (
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            />
            {filtersDirty && (
              <button onClick={() => { setQuery(""); setCatF("all"); setStatusF("all"); setRange("all"); setSortBy("newest"); }} className="h-[2.6rem] px-3 rounded-lg border border-mora-primary/15 text-sm text-mora-neutral hover:bg-mora-primary/5 inline-flex items-center gap-1.5 press">
          ...
          <DataTable
            minWidth={880}
            columns={[
              { key: "title", label: "Package", className: "font-medium text-mora-primary", render: (p) => (
                <span className="flex items-center gap-3 min-w-0">
                  <span className="w-12 h-10 rounded-lg overflow-hidden bg-mora-primary/5 shrink-0 block">
                  ...
                  <span className="block min-w-0">
                    ...
                    <span className="block text-[11px] text-mora-neutral/60 truncate">{p.destination || "—"}</span>
                  </span>
                </span>
              ) },
              { key: "category", label: "Category", render: (p) => {
                const Icon = categoryIcon(p.category);
                return <span className="inline-flex items-center gap-1.5 text-mora-neutral"><Icon className="w-3.5 h-3.5 text-gold shrink-0" />{categoryLabel(p.category)}</span>;
              } },
                  ...
                  <span className="inline-flex flex-col items-end">
                    ...
                    {off != null && (
                      <span className="inline-flex items-center gap-1.5 mt-0.5">
                        <span className="text-[11px] text-mora-neutral/50 line-through">{formatIDR(p.price_before)}</span>
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-mora-primary/10 text-mora-primary">-{off}%</span>
                      </span>
                    )}
                  </span>
                );
              ...
              { key: "actions", label: "", align: "right", className: "text-right", render: (p) => (
                <span className="inline-flex gap-1.5">
                  {can(role, "promotions", "edit") && (
                    <button onClick={(e) => { e.stopPropagation(); setEditing(toForm(p)); }} aria-label={`Edit ${p.title || "package"}`} className="w-9 h-9 rounded-lg hover:bg-mora-primary/5 inline-flex items-center justify-center text-mora-primary hover:text-gold press"><Pencil className="w-4 h-4" /></button>
                  )}
        ...
        {editing && (
          <div className="space-y-3">
            ...
            <Fld label="Itinerary">
              ...
              <Hint>One day per line, in the format <span className="font-mono text-mora-primary">Day title :: detail</span>. Days are numbered automatically from the line order.</Hint>
            </Fld>
