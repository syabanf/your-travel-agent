...
export default function DashboardPackageDetail() {
  ...
  const back = (
    <Link to="/dashboard/packages" className="inline-flex items-center gap-1.5 text-sm text-mora-neutral hover:text-gold mb-5">
    ...
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="bg-white rounded-2xl border border-mora-primary/10 p-10 text-center">
          <h1 className="text-xl font-display font-bold text-mora-primary">Package not found</h1>
          <p className="text-sm text-mora-neutral mt-1">This package may have been removed.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      {/* Header */}
      <div className="bg-white rounded-2xl border border-mora-primary/10 p-6 flex flex-col gap-4 sm:flex-row sm:items-start">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="inline-flex items-center gap-1.5 text-[10px] px-2 py-0.5 rounded-full font-medium bg-mora-gold/10 text-gold">
          ...
          {item.destination && (
            <p className="text-sm text-mora-neutral mt-1 inline-flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-gold" /> {item.destination}</p>
          )}
          {item.summary && <p className="text-sm text-mora-neutral leading-relaxed mt-2">{item.summary}</p>}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {can(role, "promotions", "edit") && (
            <button onClick={() => navigate(`/dashboard/packages?edit=${id}`)} className="text-mora-primary bg-mora-primary/5 hover:bg-mora-primary/10 rounded-xl px-3.5 py-2 text-sm font-medium inline-flex items-center gap-1.5">
      ...
      {item.description && (
        <Card title="About this package">
          <p className="text-sm text-mora-neutral leading-relaxed whitespace-pre-line">{item.description}</p>
        </Card>
      )}
      ...
      {highlights.length > 0 && (
        <Card title="Highlights">
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {highlights.map((h, i) => (
              <li key={i} className="flex items-start gap-2.5 text-sm text-mora-primary">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
          {includes.length > 0 && (
            <div className="bg-white rounded-2xl border border-mora-primary/10 p-6">
              <h2 className="font-display font-semibold text-lg text-mora-primary mb-4">What's included</h2>
          ...
          {excludes.length > 0 && (
            <div className="bg-white rounded-2xl border border-mora-primary/10 p-6">
              <h2 className="font-display font-semibold text-lg text-mora-primary mb-4">Not included</h2>
              <ul className="space-y-2">
                {excludes.map((v, i) => (
            ...
            {itinerary.map((d, i) => (
              <li key={i} className="flex gap-4">
                <div className="shrink-0 w-11 text-center">
                  <div className="text-[10px] text-mora-neutral uppercase tracking-wider">Day</div>
                  <div className="text-lg font-display font-bold text-gold leading-none">{d.day ?? i + 1}</div>
                </div>
                <div className="min-w-0 flex-1 border-l border-mora-primary/10 pl-4 pb-1">
                  <p className="text-sm font-semibold text-mora-primary">{d.title || `Day ${d.day ?? i + 1}`}</p>
                  {d.detail && <p className="text-sm text-mora-neutral leading-relaxed mt-0.5">{d.detail}</p>}
                </div>
              </li>
            ))}
          </ol>
        ...
        <Card title="Upcoming departures" icon={CalendarClock}>
          <div className="flex flex-wrap gap-2">
            {departures.map((d, i) => {
              ...
              return (
                <span key={i} className="inline-flex items-center gap-2 rounded-xl border border-mora-primary/10 bg-mora-primary/[0.02] px-3 py-2">
                  <CalendarDays className="w-3.5 h-3.5 text-gold shrink-0" />
                  <span className="text-sm text-mora-primary font-medium">{m.isValid() ? m.format("MMM D, YYYY") : String(d)}</span>
                  {days != null && (
                    <span className="text-[11px] text-mora-neutral/60 inline-flex items-center gap-1">
