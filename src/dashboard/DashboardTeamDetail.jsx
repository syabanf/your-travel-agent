...
export default function DashboardTeamDetail() {
  ...
  const back = (
    <Link to="/dashboard/team" className="inline-flex items-center gap-1.5 text-sm text-mora-neutral hover:text-mora-primary mb-4">
    ...
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="bg-white rounded-2xl border border-mora-primary/10 p-10 text-center">
          <h1 className="text-xl font-display font-bold text-mora-primary">Member not found</h1>
          <p className="text-sm text-mora-neutral mt-1">This team member may have been removed.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="bg-white rounded-2xl border border-mora-primary/10 p-6 flex items-start gap-5">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="text-2xl font-display font-bold text-mora-primary truncate">{m.name || "Unnamed member"}</h1>
          {m.email && <p className="text-sm text-mora-neutral mt-0.5 truncate">{m.email}</p>}
      ...
      <div className="bg-white rounded-2xl border border-mora-primary/10 p-6 mt-6">
        ...
        <div className="space-y-1">
          <DetailRow icon={Mail} label="Email" value={m.email ? <a href={`mailto:${m.email}`} className="text-mora-primary hover:text-gold break-all">{m.email}</a> : null} />
      ...
      <div className="bg-white rounded-2xl border border-mora-primary/10 overflow-hidden mt-6">
        <div className="px-5 py-4 border-b border-mora-primary/5 flex items-center gap-2">
          <ShieldCheck className="w-4 h-4 text-gold" />
          <div>
            <h2 className="font-display font-semibold text-mora-primary">Permissions · {roleLabel(roleKey)}</h2>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {RESOURCES.map((res) => {
                ...
                return (
                  <tr key={res} className="border-b border-mora-primary/5 last:border-0">
                    <td className="px-5 py-3 font-medium text-mora-primary capitalize whitespace-nowrap">
                      <span className="inline-flex items-center gap-1.5">
                        {anyAllowed ? <CheckSquare className="w-3.5 h-3.5 text-gold" /> : <ShieldOff className="w-3.5 h-3.5 text-mora-neutral/30" />}
                        {res}
                      </span>
                    </td>
                    {ACTIONS.map((a) => {
                      ...
                      return (
                        <td key={a.key} className="px-3 py-3 text-center">
                          <span
                            ...
                            className={`font-mono text-xs ${allowed ? "text-gold font-semibold" : "text-mora-neutral/25 line-through"}`}
                          >
                            {a.letter}
                          </span>
                        </td>
