...
export default function DashboardHeader({ onMenu, role, groups }) {
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/85 backdrop-blur-md border-b border-ich-primary/10">
      <div className="h-16 flex items-center gap-3 px-4 lg:px-6">
        {/* Mobile menu */}
        <button onClick={onMenu} aria-label="Open menu" className="lg:hidden w-10 h-10 -ml-1 rounded-xl hover:bg-ich-primary/5 flex items-center justify-center text-ich-primary shrink-0">
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <nav className="flex items-center gap-1.5 text-sm min-w-0">
            <span className="text-ich-neutral/60 hidden sm:inline">{active?.group || "Dashboard"}</span>
            <ChevronRight className="w-3.5 h-3.5 text-ich-neutral/30 hidden sm:inline shrink-0" />
            <span className="font-display font-semibold text-ich-primary truncate">{active?.label || "Overview"}</span>
          </nav>
        </div>
        ...
        <div ref={boxRef} className="relative">
          <form onSubmit={onSubmit} className="hidden md:block">
            <Search className="w-4 h-4 text-ich-neutral/50 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
            <input
              value={q}
          ...
          <button onClick={() => setOpenSearch((v) => !v)} aria-label="Search" className="md:hidden w-10 h-10 rounded-xl hover:bg-ich-primary/5 flex items-center justify-center text-ich-neutral">
            <Search className="w-5 h-5" />
          </button>

          {openSearch && (
            <div className="absolute right-0 mt-2 w-72 max-w-[80vw] bg-white rounded-2xl border border-ich-primary/10 shadow-xl overflow-hidden z-50">
              <div className="md:hidden p-2 border-b border-ich-primary/10">
              ...
              <ul className="max-h-72 overflow-y-auto py-1">
                {results.length ? results.map((it) => (
                  <li key={it.to}>
                    <button onClick={() => goto(it.to)} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm hover:bg-ich-primary/5 text-left">
                      <it.icon className="w-4 h-4 text-gold shrink-0" />
                      <span className="text-ich-primary truncate">{it.label}</span>
                      <span className="ml-auto text-[10px] uppercase tracking-wider text-ich-neutral/40 shrink-0">{it.group}</span>
                    </button>
                  </li>
                )) : (
              ...
              <div className="hidden md:flex items-center gap-1.5 px-3 py-2 border-t border-ich-primary/10 text-[11px] text-ich-neutral/50">
        ...
        {/* Recent activity */}
        <Link to="/dashboard/audit" aria-label="Activity log" className="relative w-10 h-10 rounded-xl hover:bg-ich-primary/5 flex items-center justify-center text-ich-neutral shrink-0">
          <Bell className="w-5 h-5" />
          <span className="absolute top-2.5 right-2.5 w-1.5 h-1.5 bg-ich-gold rounded-full" />
        </Link>

        {/* Role + avatar */}
        <div className="flex items-center gap-2 shrink-0">
          <span className="hidden sm:inline text-xs font-medium text-ich-neutral capitalize">{roleLabel}</span>
