...
export default function ViewToggle({ value, onChange }) {
  const opt = (key, Icon, label) => (
    <button
      ...
      className={`flex items-center gap-1.5 px-3 h-[2.6rem] rounded-lg text-sm font-medium transition-colors ${
        value === key ? "bg-mora-gold/10 text-gold" : "text-mora-neutral hover:bg-mora-primary/5"
      }`}
    >
  ...
  return (
    <div className="flex items-center gap-1 p-1 rounded-xl border border-mora-primary/10 bg-white shrink-0">
