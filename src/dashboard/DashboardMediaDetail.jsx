...
export default function DashboardMediaDetail() {
  ...
  const back = (
    <Link to="/dashboard/media" className="inline-flex items-center gap-1.5 text-sm text-mora-neutral hover:text-mora-primary mb-4">
    ...
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="bg-white rounded-2xl border border-mora-primary/10 p-10 text-center">
          <h1 className="text-xl font-display font-bold text-mora-primary">Media not found</h1>
          <p className="text-sm text-mora-neutral mt-1">This asset may have been removed.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-mora-primary/10 overflow-hidden">
          <div className="aspect-video bg-mora-primary/5 relative flex items-center justify-center">
            {m.url && (
              <img
                src={m.url}
                alt={m.title}
        ...
        {/* Details */}
        <div className="bg-white rounded-2xl border border-mora-primary/10 p-6 flex flex-col">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-2xl bg-mora-gold/10 text-gold flex items-center justify-center shrink-0"><ImageIcon className="w-6 h-6" /></div>
            <div className="min-w-0 flex-1">
              <h1 className="text-2xl font-display font-bold text-mora-primary break-words">{m.title || "Untitled asset"}</h1>
              {m.created_date && <p className="text-sm text-mora-neutral/70 mt-0.5">Added {moment(m.created_date).format("D MMM YYYY")}</p>}
            </div>
          ...
          <div className="mt-5 space-y-1">
            <DetailRow icon={Link2} label="URL" value={m.url ? <a href={m.url} target="_blank" rel="noreferrer" className="text-mora-primary hover:text-gold break-all">{m.url}</a> : null} />
            ...
            <DetailRow icon={Tag} label="Tags" value={tags.length ? (
              <span className="flex flex-wrap gap-1.5">
                {tags.map((t, i) => <span key={i} className="text-[11px] bg-mora-primary/5 text-mora-neutral rounded-full px-2 py-0.5">{t}</span>)}
              </span>
            ) : null} />
          </div>

          <div className="flex flex-wrap gap-2 mt-auto pt-6">
            <button onClick={copyUrl} disabled={!m.url} className="rounded-xl px-4 py-2.5 text-sm font-semibold flex items-center gap-2 border border-mora-primary/15 text-mora-primary hover:bg-mora-primary/5 press disabled:opacity-50">
